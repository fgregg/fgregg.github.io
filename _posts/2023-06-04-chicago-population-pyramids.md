...
This notebook shows the [population pyramids](https://en.wikipedia.org/wiki/Population_pyramid) for different racial and ethnic groups in Chicago. The default year is for ${last_year}, but you can select previous years back to 2005.<sup>*</sup>
...
```js
const year_total = view(
  Inputs.range([2005, last_year], {
    label: "Year",
    step: 1,
    value: last_year,
  }),
...
```js
const ages_san_asians = ages.filter(
  (d) => d.race !== "Asian" && d.race != "total",
);
```
...
```js
const black_ages_raw = censusChicagoGroupYears(
  "B01001B",
  2005,
  last_year,
  true,
...
```js
const white_ages_raw = censusChicagoGroupYears(
  "B01001H",
  2005,
  last_year,
  true,
...
```js
const latino_ages_raw = censusChicagoGroupYears(
  "B01001I",
  2005,
  last_year,
  true,
...
```js
const asian_ages_raw = censusChicagoGroupYears(
  "B01001D",
  2005,
  last_year,
  true,
...
```js
const total_ages_raw = censusChicagoGroupYears("B01001", 2005, last_year, true);
```
