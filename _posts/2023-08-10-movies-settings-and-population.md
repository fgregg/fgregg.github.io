...
The cities above the black line are the settings for more movies than would be
expected for their population. The cities below are the settings for fewer
movies than expected.

Hover over a dot to see the city's details.

The data comes from mighty
[wikidata](https://www.wikidata.org/wiki/Wikidata:Main_Page). Settings for only
one movie are omitted.
